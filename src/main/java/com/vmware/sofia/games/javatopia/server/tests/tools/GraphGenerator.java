...
public class GraphGenerator
{
    ...
    public static Graph generateGraph(int elements, long maxNode)
    {
        ...
        Long unlinkedElementsRandom[] = new Long[elements - rootElements.length - linkedElements.length - unlinkedElementsCyclic1.length - unlinkedElementsCyclic2.length];
        ...
        for(int i = 0; i < unlinkedElementsCyclic1.length; i++)
            unlinkedElementsCyclic1[i] = getNextUniqueRandom(maxNode, rnd, usedRandoms);

        for(int i = 0; i < unlinkedElementsCyclic2.length; i++)
            unlinkedElementsCyclic2[i] = getNextUniqueRandom(maxNode, rnd, usedRandoms);

        for(int i = 0; i < unlinkedElementsRandom.length; i++)
            unlinkedElementsRandom[i] = getNextUniqueRandom(maxNode, rnd, usedRandoms);
        ...
        addLinksInArray(unlinkedElementsRandom.length * 2, g, unlinkedElementsRandom);
        addLinksBetweenArrays(unlinkedElementsRandom.length, g, unlinkedElementsRandom, linkedElements);
        addLinksBetweenArrays(rootElements.length * 2, g, unlinkedElementsCyclic1, rootElements);
        addLinksBetweenArrays(unlinkedElementsCyclic2.length / 2, g, unlinkedElementsCyclic2, unlinkedElementsCyclic1);
        for(int i = 0; i < unlinkedElementsCyclic1.length; i++)
            g.addLink(unlinkedElementsCyclic1[i], unlinkedElementsCyclic1[(i + 1) % unlinkedElementsCyclic1.length]);

        for(int i = 0; i < unlinkedElementsCyclic2.length; i++)
            g.addLink(unlinkedElementsCyclic2[i], unlinkedElementsCyclic2[(i + 1) % unlinkedElementsCyclic2.length]);

        return g;
    }

    private static Long getNextUniqueRandom(long maxNode, Random rnd, HashSet usedRandoms)
    {
        Long res;
        do
