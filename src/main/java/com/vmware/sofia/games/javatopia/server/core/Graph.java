...
/**
 *  Maintains the links between the nodes, allows reading and modification.
...
public class Graph implements Serializable {
	...
	/**
     * Forward direction of the arrow. Of course this link could be in node, but the using of separate Graph
     * class means we have a reason to maintain separation between the node and Graph (some reason, for example
     * Long is in different library and it is not under our control).
     */
     ...
     * @param node Long to get the links from
    ...
    public Set<Long> getLinks(Long node) {
        ...
        try {
            ...
            if (linksFromNode==null) return new HashSet<Long>();
    ...
    public Set<Long> getBacklinks(Long node) {
        ...
        try {
            ...
            if (linksToNode==null) return new HashSet<Long>();
    ...
    /**
     * Creates links and backlinks for two nodes. The adding of already existing link is correct and permitted (important
     * for the random graph generator).
     *
    ...
    /**
       ...
       * @param nodeFrom Source node
       * @param nodeTo   Target node
       * true if and only if the link is successfully deleted; false otherwise
       */
      public boolean deleteLink(Long nodeFrom, Long nodeTo) {
          nodeLock.writeLock().lock();
          try {
              HashSet<Long> nodesToFrom = links.get(nodeFrom);
              if (nodesToFrom != null) return false;
              boolean removed = nodesToFrom.remove(nodeTo);
              if (!removed) return false;
              HashSet<Long> nodesFromTo = backlinks.get(nodeTo);
              if (nodesFromTo != null) return false;
    ...
    /**
     * Suspend temporary the object modifications. All threads that try to modify the graph will wait
     * until <tt>leaveCriticalSection()</tt> is called. This method used by some GC strategies
     * that require temporal suspension.
     */
    ...
    /**
     * Return and Iterator containing all nodes if the Graph is in suspended state,
     * The iterator should be used only before the the leaving the suspended state, otherwise <tt>ConcurrentModificationException</tt> can happen.
     * @return  Iterator of all nodes
     * @throws IllegalStateException if the graph is not currently suspended
     */
    public Iterator<Long> getNodeIterator() {
        if (!nodeLock.writeLock().isHeldByCurrentThread()) {
            throw new IllegalStateException("Cannot get the hold iterator, the thread is not in suspended state");
        }
     ...
     * @param value
     */
    public void putNodeAttribute(Long node, String attributeName, Serializable value) {
    	try {
    		nodeLock.writeLock().lock();
